...
class ShowProduct extends Component {
  ...
  async componentDidMount() {
    await axios.get("http://127.0.0.1:8000/api/get-product-laza").then((res) => {
  ...
  async onSubmitHandle(e) {
    ...
    try {
      const result = await axios.post("http://127.0.0.1:8000/api/add-product-laza", {
        name: $("#inputName").val(),
      ...
      console.error("Thêm không thành công");
    }
  ...
  async deleteProduct(id) {
    if (window.confirm(`Bạn muốn xóa sản phẩm có id là ${id}`)) {
      await axios
        .delete(`http://127.0.0.1:8000/api/delete-product-laza/${id}`, {})
        .then((res) => {
  ...
  async submitEditProduct(e) {
    ...
    await axios
      .put(`http://localhost:8000/api/edit-product-laza/${id}`, {
        name: $("#editName").val(),
  ...
  async editProduct(id) {
    ...
    $("#editDescription").val(product.descrpition);
